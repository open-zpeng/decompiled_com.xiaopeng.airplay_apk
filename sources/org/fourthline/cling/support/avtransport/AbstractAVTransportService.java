...
public abstract class AbstractAVTransportService implements LastChangeDelegator {
    ...
    public void appendCurrentState(LastChange lc, UnsignedIntegerFourBytes instanceId) throws Exception {
        ...
        if (mediaInfo.getNextURI() != null) {
            lc.setEventedValue(instanceId, new AVTransportVariable.NextAVTransportURI(URI.create(mediaInfo.getNextURI())));
        }
        if (positionInfo.getTrackURI() != null) {
            lc.setEventedValue(instanceId, new AVTransportVariable.CurrentTrackURI(URI.create(positionInfo.getTrackURI())));
        }
